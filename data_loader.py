...
def get_data(id_dict):
    ...
    t = time.time()
    for key, value in id_dict.items():
        train_data += [imageio.imread('IMagenet/tiny-imagenet-200/train/{}/images/{}_{}.JPEG'.format(key, key, str(i)), pilmode='RGB') for i in range(450)]
        
        train_labels_ = np.array([[0]*200]*450)
        train_labels_[:, value] = 1
        train_labels += train_labels_.tolist()
        
        val_data += [imageio.imread('IMagenet/tiny-imagenet-200/train/{}/images/{}_{}.JPEG'.format(key, key, str(i)), pilmode='RGB') for i in range(450, 500)]
    ...
    for line in open('IMagenet/tiny-imagenet-200/val/val_annotations.txt'):
        ...
        test_data.append(imageio.imread('IMagenet/tiny-imagenet-200/val/images/{}'.format(img_name), pilmode='RGB'))
